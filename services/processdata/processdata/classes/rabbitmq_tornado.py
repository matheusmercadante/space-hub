...
class TornadoAdapter:
    def __init__(self, rabbitmq_url, configuration, io_loop=None):
        ...
        self._publish_connection = RabbitmqConnection(
            rabbitmq_url, io_loop, self.logger)
        self._publish_channels = {
            publish_configuration["exchange"]: RabbitmqChannel(
        ...
        self._receive_connection = RabbitmqConnection(
            rabbitmq_url, io_loop, self.logger)
        self._receive_channels = {
            receive_configuration["queue"]: RabbitmqChannel(
    ...
    def _on_message(self, unused_channel, basic_deliver, properties, body, handler=None):
        ...
        self._io_loop.call_later(
            0.01, self._process_message, unused_channel, basic_deliver, properties, body, handler)
    ...
    def _rpc_callback_process(self, unused_channel, basic_deliver, properties, body):
        self.logger.info(
            f"RPC get response, correlation id: {properties.correlation_id}")
        if properties.correlation_id in self._rpc_corr_id_dict:
            self.logger.info(
                f"RPC get response, correlation id: {properties.correlation_id} was found in state dict")
            self._rpc_corr_id_dict[properties.correlation_id].set_result(body)
        else:
            self.logger.warning(
                f"RPC get non exist response. Correlation id: {properties.correlation_id}")
        unused_channel.basic_ack(basic_deliver.delivery_tag)
        ...
        def on_timeout():
            if corr_id in self._rpc_corr_id_dict:
                ...
                future.set_exception(
                    Exception(f'RPC timeout. Correlation id: {corr_id}'))
