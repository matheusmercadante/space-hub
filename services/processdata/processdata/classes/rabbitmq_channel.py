...
class RabbitmqChannel:
    ...
    def consume(self, on_message_callback, handler=None, no_ack=False):
        self.logger.info(
            f"[start consuming] routing key: {self._routing_key}; queue name: {self._queue}")
        ...
        if handler is not None:
            ...
            channel.basic_consume(
                queue=self._queue, on_message_callback=on_message_callback, auto_ack=no_ack)
    ...
    def publish(self, body, mandatory=None, properties=None, reply_to=None):
        ...
        self.logger.info(
            f"Publishing message. exchange: {exchange}; routing_key: {routing_key}")
    ...
    def _create_channel(self):
        ...
        def on_channel_closed(channel, reason):
            ...
            if reply_code not in [self._NORMAL_CLOSE_CODE, self._USER_CLOSE_CODE]:
                ...
                self.logger.info(
                    f"Reply code: {reply_code}, reply text: {reply_txt}")

        def on_channel_return(channel, method, property, body):
            """"If publish message has failed, this method will be invoked."""
            self.logger.error(
                f"Rejected from server. reply code: {method.reply_code}, reply text: {method.reply_txt}")
    ...
    def _queue_bind(self):
        self.logger.info(
            f"Binding queue: {self._queue} to exchange: {self._exchange}")
    ...
    def _on_queue_bind_ok(self, unframe):
        self.logger.info(
            f"bound queue: {self._queue} to exchange: {self._exchange}")
        self._on_setup_complete()
